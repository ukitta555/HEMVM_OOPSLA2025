...
use jsonrpc_core::{BoxFuture, Error as JsonRpcError, Error, Result as JsonRpcResult};
use network::{
...
use rlp::Rlp;
use rustc_hex::ToHex;
use std::{collections::BTreeMap, net::SocketAddr, sync::Arc, thread, time, time::Duration};
...
impl RpcImpl {
    ...
    ) -> Self
    {
     ...
     fn load_test(&self) -> RpcResult<U256> {
        println!("NEW!");
        let bytes = std::fs::read("../client/src/rpc/impls/transactions.txt").unwrap();
        let mut txs: Vec<Vec<u8>> = Vec::with_capacity(100000);
        let mut counter = 0;
        let mut current_byte: usize = 0;
        let mut last_hash: H256 = H256::zero();
        while current_byte < bytes.len() {
            let first_byte = (bytes[current_byte + 1] as u32) << 8;
            let tx_length: u32 = first_byte + bytes[current_byte] as u32;
        ...
        for eth_tx in txs {
            ...
            //     println!("I sleep for 1500 ms...");
            // }
            self.send_raw_transaction(
                Bytes::new(eth_tx)
            ).expect("not okey-dockey :(");
            // println!("Upload tx {} to mempool", i);
            i = i + 1;
        }
        Ok(U256::from(1))
    }
