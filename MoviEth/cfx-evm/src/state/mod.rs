...
use cfx_state::{
    state_trait::{AsStateOpsTrait, CallMoveVMTrait, CheckpointTrait, StateOpsTrait},
...
pub struct State<'a> {
    db: StateDb<'a>,
    move_vm: Option<&'a mut dyn CallMoveVMTrait>,
...
impl<'a> StateOpsTrait for State<'a> {
    fn as_move_vm(&mut self) -> Option<&mut dyn CallMoveVMTrait> {
        match (&mut self.move_vm).as_mut() {
            Some(vm) => Some(&mut *vm),
            None => None,
        }
    }
...
impl<'a> State<'a> {
    ...
    pub fn new_with_move_vm(
        db: StateDb<'a>,
        move_vm: &'a mut dyn CallMoveVMTrait,
    ) -> DbResult<Self> {
        let total_issued_tokens = db.get_total_issued_tokens()?;
    ...
    {
        let mut cache;
        if !self.cache.read().contains_key(address) { // account isn't in cache of EVM state
            let account = self
                .db
                .get_account(address)?
                .map(|acc| OverlayAccount::from_loaded(address, acc));
            ...
            cache = self.cache.write(); // account is in cache of EVM state
        };
        ...
        if entry.account.is_none() {
            entry.account = Some(default(address)?); // create default account in case it wasn't populated
        }
