...
fn load_solidity_fn<'a>(
    ...
    let solidity_fn = func_table.get(&fn_sig);
    let solidity_fn = solidity_fn.filter(|&func| func.is_active(spec));
    let solidity_fn = solidity_fn.ok_or(vm::Error::InternalContract("unsupported function".into()))?;
