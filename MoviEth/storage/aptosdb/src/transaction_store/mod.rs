...
use crate::{
    ...
    schema::{
        ...
        transaction_by_eth_hash::TransactionByEthHashSchema,
...
impl TransactionStore {
    ...
    /// Gets the version of a transaction by an Ethereum transaction hash.
    pub fn get_transaction_version_by_eth_hash(
        &self,
        hash: &HashValue,
        ledger_version: Version,
    ) -> Result<Option<Version>> {
        Ok(match self.db.get::<TransactionByEthHashSchema>(hash)? {
            Some(version) if version <= ledger_version => Some(version),
            _ => None,
        })
    }
    ...
    pub fn put_transaction(
        ...
        if let Ok(signed_tx) = transaction.as_signed_user_txn() {
            if let Some(eth_tx) = signed_tx.eth_transaction() {
                batch.put::<TransactionByEthHashSchema>(&eth_tx.hash().into(), &version)?;
            }
        }
