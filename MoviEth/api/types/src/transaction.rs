...
use aptos_crypto::{
    ...
    secp256k1,
    secp256k1::{Secp256k1PublicKey, SECP256K1_PUBLIC_KEY_LENGTH, SECP256K1_SIGNATURE_LENGTH},
};
use aptos_types::{
...
impl VerifyInput for TransactionSignature {
    fn verify(&self) -> anyhow::Result<()> {
        match self {
            ...
            TransactionSignature::Secp256k1Signature(inner) => inner.verify(),
        }
    }
}
...
impl TryFrom<TransactionSignature> for TransactionAuthenticator {
    ...
    fn try_from(ts: TransactionSignature) -> anyhow::Result<Self> {
        Ok(match ts {
            ...
            TransactionSignature::Secp256k1Signature(sig) => sig.try_into()?,
        })
    }
}
...
impl VerifyInput for Secp256k1Signature {
    fn verify(&self) -> anyhow::Result<()> {
        let public_key_len = self.public_key.inner().len();
        let signature_len = self.signature.inner().len();
        if public_key_len != SECP256K1_PUBLIC_KEY_LENGTH {
            bail!(
                "Secp256k1 signature's public key is an invalid number of bytes, should be {} bytes but found {}",
                SECP256K1_PUBLIC_KEY_LENGTH, public_key_len
            )
        } else if signature_len != SECP256K1_SIGNATURE_LENGTH {
            bail!(
                "Secp256k1 signature length is an invalid number of bytes, should be {} bytes but found {}",
                SECP256K1_SIGNATURE_LENGTH, signature_len
            )
        } else {
            // TODO: Check if they match / parse correctly?
            Ok(())
...
impl TryFrom<Secp256k1Signature> for TransactionAuthenticator {
    type Error = anyhow::Error;

    fn try_from(value: Secp256k1Signature) -> Result<Self, Self::Error> {
        let public = value.public_key.inner().try_into()?;
        let sig = value.signature.inner().try_into()?;
        Ok(TransactionAuthenticator::secp256k1(public, sig))
    }
}

impl TryFrom<Secp256k1Signature> for AccountAuthenticator {
    type Error = anyhow::Error;

    fn try_from(value: Secp256k1Signature) -> Result<Self, Self::Error> {
        let public = value.public_key.inner().try_into()?;
        let sig = value.signature.inner().try_into()?;
        Ok(AccountAuthenticator::secp256k1(public, sig))
    }
}
...
impl VerifyInput for AccountSignature {
    fn verify(&self) -> anyhow::Result<()> {
        match self {
            ...
            AccountSignature::Secp256k1Signature(inner) => inner.verify(),
        }
    }
}
...
impl TryFrom<AccountSignature> for AccountAuthenticator {
    ...
    fn try_from(sig: AccountSignature) -> anyhow::Result<Self> {
        Ok(match sig {
            ...
            AccountSignature::Secp256k1Signature(s) => s.try_into()?,
        })
    }
}
...
impl From<(&Secp256k1PublicKey, &secp256k1::Secp256k1Signature)> for Secp256k1Signature {
    fn from((pk, sig): (&Secp256k1PublicKey, &secp256k1::Secp256k1Signature)) -> Self {
        Self {
            public_key: pk.to_bytes().to_vec().into(),
            signature: sig.to_bytes().to_vec().into(),
        }
    ...
    fn from(auth: &AccountAuthenticator) -> Self {
        ...
        match auth {
            ...
            Secp256k1 {
                public_key,
                signature,
            } => Self::Secp256k1Signature((public_key, signature).into()),
        }
    }
}
