...
use aptos_block_executor::move_vm_ext::MoveResolverExt;
...
impl<'a, R: MoveResolverExt + ?Sized> MoveConverter<'a, R> {
    pub fn new(inner: &'a R, db: Arc<DbReaderWriter>) -> Self {
        Self {
    ...
    pub fn try_into_transaction_payload(
        ...
        let ret = match payload {
            ...
            EthTransactionPayload(_) => {
                bail!("unsupported Eth transaction")
            },
        };
        Ok(ret)
    }
    ...
    pub fn try_access_path_into_write_set_change(
        ...
        let ret = match op {
            WriteOp::Deletion => match access_path.get_path() {
                ...
                Path::Evm(_) => {
                    // TODO(zeroxg): Support output EVM change set
                    return Err(anyhow!("Not support Evm set yet"));
                },
            },
    ...
    pub fn try_write_table_item_into_decoded_table_data(
        ...
        if !self.db.reader.indexer_enabled() {
            return Ok(None);
        }
        let table_info = self.db.reader.get_table_info(handle).unwrap();
...
pub trait AsConverter<R> {
    fn as_converter(&self, db: Arc<DbReaderWriter>) -> MoveConverter<R>;
}
