...
use aptos_config::config::{ApiConfig, EthApiConfig, DEFAULT_MAX_PAGE_SIZE};
...
async fn main() {
    ...
    // TODO(lpl): Enable Eth RPCs here with a different context?
...
trait ServerArgs {
    ...
    fn eth_api_config(&self) -> EthApiConfig;
...
impl ServerArgs for CommandArgs {
    ...
    fn eth_api_config(&self) -> EthApiConfig {
        match self {
            CommandArgs::OnlineRemote(args) => args.eth_api_config(),
            CommandArgs::Offline(args) => args.eth_api_config(),
            CommandArgs::Online(args) => args.eth_api_config(),
        }
    }
...
impl ServerArgs for OfflineArgs {
    ...
    fn eth_api_config(&self) -> EthApiConfig {
        // TODO(lpl): Support other configs.
        let mut config = EthApiConfig::default();
        config.address = self.eth_listen_address;
        config
    }
...
impl ServerArgs for OnlineRemoteArgs {
    ...
    fn eth_api_config(&self) -> EthApiConfig {
        self.offline_args.eth_api_config()
    }
...
impl ServerArgs for OnlineLocalArgs {
    ...
    fn eth_api_config(&self) -> EthApiConfig {
        self.online_args.offline_args.eth_api_config()
    }
