...
use cfxkey::{
    Generator, Message, Public, Random as EthkeyRandom, Secret, Signature as EthkeySignature,
    SECP256K1,
};
...
impl Secp256k1Signature {
    ...
    pub(crate) fn from_bytes_unchecked(
        ...
        let byte_array: [u8; 65] = bytes
            .try_into()
            .map_err(|_| CryptoMaterialError::DeserializationError)?;
        Ok(Secp256k1Signature(byte_array.into()))
    }
    ...
    fn verify_arbitrary_msg(&self, message: &[u8], public_key: &Secp256k1PublicKey) -> Result<()> {
        ...
        let msg = Message::from_slice(message.to_vec().as_slice());
    ...
    pub fn signature(&self) -> &EthkeySignature {
        &self.0
    }
}
...
impl SigningKey for Secp256k1PrivateKey {
    ...
    fn sign<T: CryptoHash + Serialize>(
        &self,
        message: &T,
    ) -> Result<Secp256k1Signature, CryptoMaterialError> {
        let mut bytes = <T::Hasher as CryptoHasher>::seed().to_vec();
        bcs::serialize_into(&mut bytes, &message)
            .map_err(|_| CryptoMaterialError::SerializationError)
            .expect("Serialization of signable material should not fail.");
        Ok(Secp256k1PrivateKey::sign_arbitrary_message(
            self,
            bytes.as_slice(),
        ))
    }
...
impl TryFrom<&[u8]> for Secp256k1PrivateKey {
    ...
    /// validity.
    fn try_from(bytes: &[u8]) -> std::result::Result<Secp256k1PrivateKey, CryptoMaterialError> {
...
impl From<&Secp256k1PrivateKey> for Secp256k1PublicKey {
    fn from(secret_key: &Secp256k1PrivateKey) -> Self {
        ...
        let s: key::SecretKey =
            key::SecretKey::from_slice(context, &secret[..]).expect("Error secret!");
        let pub_key = key::PublicKey::from_secret_key(context, &s).expect("Error public!");
...
impl TryFrom<&[u8]> for Secp256k1PublicKey {
    ...
    fn try_from(bytes: &[u8]) -> std::result::Result<Secp256k1PublicKey, CryptoMaterialError> {
...
impl Signature for Secp256k1Signature {
    ...
    fn verify<T: CryptoHash + Serialize>(
        ...
        // FIXME(lpl): Check if this path is possible.
        bail!("secp256k1 is not used to sign CryptoHash messages")
        // Public keys should be validated to be safe against small subgroup
        // attacks, etc.
        // let mut bytes = <T::Hasher as CryptoHasher>::seed().to_vec();
        // bcs::serialize_into(&mut bytes, &message)
...
impl TryFrom<&[u8]> for Secp256k1Signature {
    ...
    fn try_from(bytes: &[u8]) -> std::result::Result<Secp256k1Signature, CryptoMaterialError> {
...
/// disappear after
pub mod compat {
    use crate::{secp256k1::*, test_utils::KeyPair, Uniform};
    #[cfg(feature = "fuzzing")]
    ...
    pub fn generate_keypair<'a, T>(_opt_rng: T) -> (Secp256k1PrivateKey, Secp256k1PublicKey)
    where
    ...
    ) -> impl Strategy<Value = KeyPair<Secp256k1PrivateKey, Secp256k1PublicKey>> {
        crate::test_utils::uniform_keypair_strategy::<Secp256k1PrivateKey, Secp256k1PublicKey>()
    }
    ...
    pub fn generate_genesis_keypair() -> (Secp256k1PrivateKey, Secp256k1PublicKey) {
        let mut buf = [0u8; SECP256K1_PRIVATE_KEY_LENGTH];
        buf[SECP256K1_PRIVATE_KEY_LENGTH - 1] = 1;
