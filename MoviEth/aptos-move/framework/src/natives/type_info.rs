...
fn native_encoded_type_tag(
    ...
    let type_tag = context.type_to_type_tag(&ty_args[0])?;
    let encoded_type_tag = bcs::to_bytes(&type_tag).expect("Should success");

    Ok(NativeResult::ok(InternalGas::new(0), smallvec![
        Value::vector_u8(encoded_type_tag)
    ]))
}

pub fn make_native_encoded_type_tag() -> NativeFunction {
    Arc::new(move |context, ty_args, args| native_encoded_type_tag(context, ty_args, args))
}
...
pub fn make_all(gas_params: GasParameters) -> impl Iterator<Item = (String, NativeFunction)> {
    let natives = [
        ...
        ("encoded_type_tag", make_native_encoded_type_tag()),
        (
            "chain_id_internal",
