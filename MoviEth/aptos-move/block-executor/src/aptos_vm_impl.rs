...
use crate::{
    access_path_cache::AccessPathCache, counters::*, data_cache::StorageAdapter, errors::{convert_epilogue_error, convert_prologue_error, expect_only_successful_execution}, logging::AdapterLogSchema, move_vm_ext::{MoveResolverExt, MoveVmExt, SessionExt, SessionId}, transaction_metadata::TransactionMetadata
};
use aptos_aggregator::transaction::TransactionOutputExt;
use aptos_framework::{natives::call_evm::CrossVMContext, RuntimeModuleMetadataV1};
use aptos_gas::{
...
impl AptosVMImpl {
    ...
    pub fn new_session_with_evm_ref<'r, R: MoveResolverExt>(
        &self,
        r: &'r R,
        session_id: SessionId,
        evm_caller: CrossVMContext<'r>,
    ) -> SessionExt<'r, '_, R> {
        self.move_vm
            .new_session_with_evm_ref(r, session_id, evm_caller)
    }
