...
use crate::{
    adapter_common::preprocess_transaction, aptos_vm::AptosVM, block_executor::vm_wrapper::AptosExecutorTask, cached_state_view::CachedStateView, errors2::Error2, executor::{ExecutionSpace, TwoWayChannelEndpoints, TwoWayChannels}
};
use aptos_aggregator::{delta_change_set::DeltaOp, transaction::TransactionOutputExt};
use crate::{
    executor::{BlockExecutor, PreprocessedTransaction, RAYON_EXEC_POOL},
...
impl BlockAptosVM {
    ...
    pub fn execute_block_prototype(
    ...
    ) -> Result<Vec<TransactionOutput>, VMStatus>
    {
        ...
        let executor = BlockExecutor::<PreprocessedTransaction, AptosExecutorTask<CachedStateView>, CachedStateView>::new(
            concurrency_level,
        );

        let mut ret = 
            executor
                .execute_transactions_parallel_prototype(
                    // state_view_copy, 
        ...
        if ret == Err(Error2::ModulePathReadWrite) {
        ...
        match ret {
            ...
            Err(Error2::ModulePathReadWrite) => {
                unreachable!("[Execution]: Must be handled by sequential fallback")
            },
            Err(Error2::UserError(err)) => Err(err),
        }
    }

    pub fn execute_block<S: StateView + Sync + Send>(
        transactions: Vec<Transaction>,
        state_view: &S,
        concurrency_level: usize,
    ) -> Result<Vec<TransactionOutput>, VMStatus> {
