...
impl aptos_block_executor::state_view::DbReader for FakeDbReader {
...
impl<'a> MoveTestAdapter<'a> for AptosTestAdapter<'a> {
    ...
    fn handle_subcommand(&mut self, _input: TaskInput<Self::Subcommand>) -> Result<Option<String>> {
        Result::Ok(Option::Some(String::from("123")))
        // match input.command {
...
pub fn run_aptos_test(path: &Path) -> Result<(), Box<dyn std::error::Error>> {
    ...
    aptos_block_executor::aptos_vm::allow_module_bundle_for_test();
