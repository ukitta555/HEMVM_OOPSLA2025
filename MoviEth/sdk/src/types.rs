...
use std::str::FromStr;
use aptos_crypto::secp256k1::{Secp256k1PrivateKey, Secp256k1PublicKey};
use aptos_types::transaction::{eth_public_to_address, EthAddress};
...
#[derive(Debug)]
pub struct LocalEthAccount {
    /// Address of the Eth account.
    address: EthAddress,
...
impl LocalEthAccount {
    ...
    pub fn generate<R>(rng: &mut R) -> Self
        where
            R: ::rand_core::RngCore + ::rand_core::CryptoRng,
    {
        let private_key = Secp256k1PrivateKey::generate(rng);
        let public_key: Secp256k1PublicKey = Secp256k1PublicKey::from(&private_key);
        let address = eth_public_to_address(public_key.public());
        Self::new(
            address,
            private_key,
            public_key,
    ...
    pub fn private_key(&self) -> &Secp256k1PrivateKey { &self.private_key }
}
