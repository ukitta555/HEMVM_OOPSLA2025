...
use aptos_block_executor::executor::ExecutionContext;
...
use aptos_vm::{block_executor::{vm_wrapper::AptosExecutorTask, AptosTransactionOutput}, cached_state_view::CachedStateView, VMExecutor};
...
impl VMExecutor for MockVM {
    ...
    fn execute_block_prototype(
        transactions: Vec<Transaction>,
        state_view: Arc<S>,
        move_channel: &(Sender<ExecutionContext<AptosExecutorTask<CachedStateView>, CachedStateView>>, Receiver<AptosTransactionOutput, VMStatus>),
        eth_channel: &(Sender<ExecutionContext<AptosExecutorTask<CachedStateView>, CachedStateView>>, Receiver<AptosTransactionOutput, VMStatus>),
        cross_channel: &(Sender<ExecutionContext<AptosExecutorTask<CachedStateView>, CachedStateView>>, Receiver<AptosTransactionOutput, VMStatus>)
    ) -> Result<Vec<TransactionOutput>, VMStatus> 
    {
        Ok(Vec::new())
    }
...
fn decode_transaction(txn: &SignedTransaction) -> MockVMTransaction {
    ...
    match txn.payload() {
        ...
        TransactionPayload::EthTransactionPayload(_) => {
            unimplemented!("MockVM does not support Eth transaction payload.")
        },
    }
}
