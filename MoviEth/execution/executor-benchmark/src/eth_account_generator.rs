...
use aptos_sdk::types::LocalEthAccount;
use aptos_types::transaction::EthAddress;

type Seed = [u8; 32];

pub struct EthAccountGenerator {
    receiver: mpsc::Receiver<LocalEthAccount>,
}

impl EthAccountGenerator {
    ...
    fn new(root_seed: u64, num_to_skip: u64) -> Self {
        ...
        for _ in 0..active_rng_to_skip {
            LocalEthAccount::generate(&mut active_rng);
        }
        ...
        std::thread::Builder::new()
            ...
            .spawn(move || {
                while sender.send(LocalEthAccount::generate(&mut active_rng)).is_ok() {
                    active_rng_quota -= 1;
    ...
    pub fn generate(&mut self) -> LocalEthAccount {
        self.receiver.recv().unwrap()
    }
}

pub struct EthAccountCache {
    generator: EthAccountGenerator,
    pub accounts: VecDeque<LocalEthAccount>,
    rng: StdRng,
}

impl EthAccountCache {
    const SEED: Seed = [1; 32];

    pub fn new(generator: EthAccountGenerator) -> Self {
        Self {
            generator,
    ...
    pub fn accounts(&self) -> &VecDeque<LocalEthAccount> {
        &self.accounts
    }
    ...
    pub fn get_random(&mut self) -> &mut LocalEthAccount {
    ...
    pub fn get_random_transfer(&mut self) -> (&mut LocalEthAccount, EthAddress) {
