...
use crate::{
    ...
    transaction::{
        eth_address_to_aptos_address, eth_public_to_address, RawTransaction, RawTransactionWithData,
    },
};
...
impl TransactionAuthenticator {
    ...
    pub fn secp256k1(public_key: Secp256k1PublicKey, signature: Secp256k1Signature) -> Self {
        Self::Secp256k1 {
            public_key,
            signature,
        }
    ...
    pub fn verify(&self, raw_txn: &RawTransaction) -> Result<()> {
        ...
        match self {
            ...
            TransactionAuthenticator::Secp256k1 {
                signature,
                public_key: public,
            } => {
                // We do not allow key rotation in eth space, so we check if the sender matches
                // the public key here. The match of signature and public is checked in `verify`.
                if raw_txn.sender()
                    != eth_address_to_aptos_address(&eth_public_to_address(public.public()))
                {
                    bail!("eth tx sender and public key unmatch");
                }
                let eth_raw_tx = raw_txn
                    .eth_transaction()
                    .ok_or_else(|| anyhow!("secp256k1 signature with wrong tx type"))?;
                signature.verify_arbitrary_msg(eth_raw_tx.signature_hash().as_ref(), public)
            },
        }
    }
...
pub enum Scheme {
    Ed25519 = 0,
    MultiEd25519 = 1,
    // TODO(lpl): Should we use a larger number for better future compatibility with Aptos?
    Secp256k1 = 2,
...
impl AccountAuthenticator {
    ...
    pub fn verify<T: Serialize + CryptoHash>(&self, message: &T) -> Result<()> {
        match self {
            ...
            Self::Secp256k1 { .. } => {
                // FIXME(lpl): Check if it's possible to unmatch.
                Ok(())
            },
        }
    }
    ...
    pub fn signature_bytes(&self) -> Vec<u8> {
        match self {
            ...
            Self::Secp256k1 { signature, .. } => signature.to_bytes().to_vec(),
        }
    }
