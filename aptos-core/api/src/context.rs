...
use aptos_storage_interface::{
    ...
    DbReader, Order, MAX_REQUEST_LIMIT, DbReaderWriter,
};
use aptos_types::{
...
impl Context {
    ...
    pub fn state_view_at_version(&self, version: Version) -> Result<DbStateView> {
        self.db.reader.state_view_at_version(Some(version))
    }
    ...
    pub fn get_latest_ledger_info<E: ServiceUnavailableError>(&self) -> Result<LedgerInfo, E> {
        let maybe_oldest_version = self
            .db
            .reader.get_first_viable_txn_version()
        ...
        let (oldest_version, oldest_block_event) = self
            .db
            .reader.get_next_block_event(maybe_oldest_version)
        ...
        let (_, _, newest_block_event) = self
            .db
            .reader.get_block_info_by_version(ledger_info.ledger_info().version())
    ...
    pub fn get_state_values(
        ...
        let mut iter = self.db.reader.get_prefixed_state_value_iterator(
    ...
    pub fn get_resources_by_pagination(
        ...
        let account_iter = self.db.reader.get_prefixed_state_value_iterator(
            &StateKeyPrefix::from(address),
            prev_state_key,
            version,
        )?;
    ...
    pub fn get_block_timestamp<E: InternalError>(
        ...
        self.db
            .reader.get_block_timestamp(version)
    ...
    pub fn get_block_by_height<E: StdApiError>(
        ...
        let (first_version, last_version, new_block_event) = self
            .db
            .reader.get_block_info_by_height(height)
    ...
    pub fn get_block_by_version<E: StdApiError>(
        ...
        let (first_version, last_version, new_block_event) = self
            .db
            .reader.get_block_info_by_version(version)
    ...
    pub fn render_transactions_sequential<E: InternalError>(
        ...
        let converter = resolver.as_converter(self.db.reader.clone());
    ...
    pub fn render_transactions_non_sequential<E: InternalError>(
        ...
        let txns: Vec<aptos_api_types::Transaction> = data
            .into_iter()
            .map(|t| {
                let timestamp = self.db.reader.get_block_timestamp(t.version)?;
                let txn = converter.try_into_onchain_transaction(timestamp, t)?;
                Ok(txn)
            })
            .collect::<Result<_, anyhow::Error>>()
    ...
    pub fn get_transactions(
        ...
        let data = self
            .db
            .reader.get_transaction_outputs(start_version, limit as u64, ledger_version)?;
    ...
    pub fn get_account_transactions<E: NotFoundError + InternalError>(
        ...
        let txns = self
            .db
            .reader.get_account_transactions(
                address,
                start_seq_number,
                limit as u64,
                true,
    ...
    pub fn get_transaction_by_hash(
        ...
        self.db
            .reader.get_transaction_by_hash(hash, ledger_version, true)?
            .map(|t| self.convert_into_transaction_on_chain_data(t))
            .transpose()
    }
    ...
    pub fn get_transaction_by_version(
        ...
        self.convert_into_transaction_on_chain_data(self.db.reader.get_transaction_by_version(
            version,
            ledger_version,
            true,
        )?)
    ...
    pub fn get_accumulator_root_hash(&self, version: u64) -> Result<HashValue> {
        self.db.reader.get_accumulator_root_hash(version)
    }
