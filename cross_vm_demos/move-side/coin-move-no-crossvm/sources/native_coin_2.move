module coin_wrapper::native_coin_2 {
    ...
    struct DiemCoin2 has copy, store {}
    struct MintCapStore has key {
        cap: coin::MintCapability<DiemCoin2>,
    }

    fun init_module(account: &signer) {
        let name = string::utf8(b"Diem Coin 2");
        let symbol = string::utf8(b"DMC2");
        let decimals = 8;
    ...
    public entry fun mint(account: &signer, receiver: address, amount: u64) acquires MintCapStore {
        ...
        let coin = coin::mint<DiemCoin2>(amount, mint_cap);
