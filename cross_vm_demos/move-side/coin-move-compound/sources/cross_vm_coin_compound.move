module coin_wrapper::cross_vm_coin_compound {
    ...
    public entry fun deposit_collateral_and_borrow<CollateralCoin, BorrowCoin>(account: &signer, amount: u64) acquires CoinInfo, CapStore {
        assert!(
            amount <= coin::balance<CollateralCoin>(signer::address_of(account)),
            error::invalid_argument(BAD_TOKEN_AMOUNT),
        );
        let wrapper = x"812cBBdE09AF8214a5c3addE18Fcec9891196494";
        deposit<CollateralCoin>(account, wrapper, amount);
        // debug::print(&to_str(b"Deposited collateral to the wrapper"));

        let encoded_amount = bcs::to_bytes(&amount);

        let handler = x"812cBBdE09AF8214a5c3addE18Fcec9891196494";
        let func_name = b"handleSupplyCollateral";
        let call_cap = &borrow_global<CapStore>(@coin_wrapper).call_cap;
        let params: vector<vector<u8>> = vector[
            raw_type<CollateralCoin>(),
            raw_type<BorrowCoin>(),
            encoded_amount
        ];

        cross_vm::call_evm(option::none(), handler, string::utf8(func_name), params, call_cap);
        // debug::print(&to_str(b"Performed cross-space supply of collateral"));

        withdraw<BorrowCoin>(@coin_wrapper);
    }

    public entry fun repay_debt_and_fetch_collateral<CollateralCoin, BorrowCoin>(account: &signer, amount: u64) acquires CoinInfo, CapStore {
        assert!(
            amount <= coin::balance<BorrowCoin>(signer::address_of(account)),
            error::invalid_argument(BAD_TOKEN_AMOUNT),
        );
        let evm_proxy = x"812cBBdE09AF8214a5c3addE18Fcec9891196494";
        deposit<BorrowCoin>(account, evm_proxy, amount);
        // debug::print(&to_str(b"Sent back borrowed tokens"));

        let encoded_amount = bcs::to_bytes(&amount);

        let handler = x"812cBBdE09AF8214a5c3addE18Fcec9891196494";
        let func_name = b"handleRepayDebt";
        let call_cap = &borrow_global<CapStore>(@coin_wrapper).call_cap;
        let params: vector<vector<u8>> = vector[
            raw_type<CollateralCoin>(),
            raw_type<BorrowCoin>(),
        ...
        cross_vm::call_evm(option::none(), handler, string::utf8(func_name), params, call_cap);
        // debug::print(&to_str(b"Performed cross-space repay of borrow"));

        withdraw<CollateralCoin>(@coin_wrapper);
    }
