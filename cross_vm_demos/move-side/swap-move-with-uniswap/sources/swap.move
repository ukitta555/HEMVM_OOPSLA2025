module swap_wrapper::swap {
    ...
    use coin_wrapper::cross_vm_coin_reverse_demo;
    ...
    fun register_for_pool_inner<X,Y>() acquires CapStore {
        ...
        cross_vm_coin_reverse_demo::initialize_vault_coin<LPToken<X,Y>>();

        let address = cross_vm_coin_reverse_demo::evm_token_address<LPToken<X,Y>>();
        let handler = x"13157441585494b5E09E066e69C779aEa08d164B";
        let func_name = b"handleSetLPToken";
        let params = vector[address, cross_vm_coin_reverse_demo::raw_type<X>(), cross_vm_coin_reverse_demo::raw_type<Y>()];
    ...
    fun sweep_out<CoinType>() acquires CapStore {
        ...
        let receiver = cross_vm_coin_reverse_demo::cashier_address();
   ...
   fun ihe_swap_exact_input<X,Y>(caller: vector<u8>, message: vector<vector<u8>>): vector<u8> acquires CapStore {
        assert!(caller==x"13157441585494b5E09E066e69C779aEa08d164B", ONLY_SWAP_CALLER);
