...
async function main() {
  ...
  console.log(`LP token ${name} (${symbol}) address ${lpAddress}`);

  await eth.approve(swap.address, toUnit(1000000, 18));
  await emc.approve(swap.address, toUnit(1000000, 18));

  let tx = await swap.addLiquidity(
    eth.address,
    emc.address,
    toUnit(1000000, 18),
    toUnit(1000000, 18),
