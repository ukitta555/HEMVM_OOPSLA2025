...
interface ICrossVM {
    ...
    function callMove(bytes32 addr, string calldata module, string calldata func, bytes[] calldata data, TypeInfo[] calldata types) external payable returns (bytes memory);

    function log(bytes memory data) external view;
