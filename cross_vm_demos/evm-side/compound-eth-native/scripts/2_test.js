...
async function main() {
  ...
  const FaucetToken2 = await ethers.getContractAt("FaucetToken2", contractAddress.FaucetToken2, deployer);

  // approve amount to transfer
  console.log("Approve tokens before minting...");
  const amt = ethers.utils.parseEther("1000");
  let tx = await FaucetToken2.approve(CErc20Twin.address, amt);
  await tx.wait();
  console.log(">>> Done");

  // add liquidity to the market
  // (Vlad) ... <-> supply FaucetToken2 for CFaucetToken2
  console.log(`Supply FaucetToken2 liquidity to the Compound, amount ${amt}`);
