...
contract MoveSwapRouter {
    CrossVmProxyReverse immutable proxy;
    ...
    constructor(address _proxy) {
        proxy = CrossVmProxyReverse(_proxy);
    }

    function approvePortal(address token) public {
        ICrossVmErc20 portal = proxy.tokenToPortal(token);
        IERC20(token).approve(address(portal), type(uint256).max);
    }
    ...
    function _swap(
        ...
        ICrossVmErc20 portalIn = proxy.tokenToPortal(address(tokenIn));
        ICrossVmErc20 portalOut = proxy.tokenToPortal(address(tokenOut));

        TokenInfo memory tokenInfoIn = proxy.getTokenInfo(address(portalIn));
        TokenInfo memory tokenInfoOut = proxy.getTokenInfo(address(portalOut));
        ...
        portalIn.deposit(cashier, amountIn);
        ...
        if (!exactIn) {
            portalIn.withdraw(msg.sender);
        }
        portalOut.withdraw(msg.sender);
    }
    ...
    function removeLiquidity(address tokenLP, uint amount) external {
        ...
        ICrossVmErc20 portalX = proxy.tokenToPortal(pool.tokenX);
        ICrossVmErc20 portalY = proxy.tokenToPortal(pool.tokenY);
        ICrossVmErc20 portalLp = proxy.tokenToPortal(tokenLP);

        IERC20(tokenLP).transferFrom(msg.sender, address(this), amount);
        portalLp.deposit(cashier, amount);

        bytes[] memory data;
        bytes[] memory coins;

        {
            TokenInfo memory tokenInfoX = proxy.getTokenInfo(address(portalX));
            TokenInfo memory tokenInfoY = proxy.getTokenInfo(address(portalY));
        ...
        portalX.withdraw(msg.sender);
        portalY.withdraw(msg.sender);
    }
    ...
    function addLiquidity(
        ...
        ICrossVmErc20 portalX = proxy.tokenToPortal(address(tokenX));
        ICrossVmErc20 portalY = proxy.tokenToPortal(address(tokenY));
        ICrossVmErc20 portalLp = proxy.tokenToPortal(lpToken);

        tokenX.transferFrom(msg.sender, address(this), amountX);
        portalX.deposit(cashier, amountX);
        tokenY.transferFrom(msg.sender, address(this), amountY);
        portalY.deposit(cashier, amountY);
