...
from local_abis import eth_coin_abi, move_router_abi
...
def generate_transfer_txs_move(
    ...
    def move_pancake_case(coin: float):
        return 0 <= coin < 0.5
    ...
    for idx in range(move_txs_counter):
        ...
        elif move_pancake_case(salad_random_coin):
...
def generate_transfer_txs_parallel_eth(
    ...
    def eth_cross_pancake_case(coin: float):
        return 0 <= coin < 1/6

    def eth_erc20_case(coin: float):
        return 1/6 <= coin < 1

    SwapETH = w3.eth.contract(address="0x13157441585494b5E09E066e69C779aEa08d164B", abi=move_router_abi)
    ...
    for idx in range(eth_tx_count):
        ...
        if eth_erc20_case(salad_random_coin):
        ...
        elif eth_cross_pancake_case(salad_random_coin):
            unsent_txn = SwapETH.functions.swapExactTokensForTokens(
                ...
                'gas': 30000000,
                'gasPrice': 1,
                "nonce": eth_nonces[sender_eth],  # + 1 for the approve tx
            })
...
async def generate_txs():
    ...
    with open(f"./transaction_batches/salad_pancake_custom15_pancake15_erc60_crosspan10_{num_of_txs // K}k.txt", "wb") as f:
        for _ in range(num_of_txs):
