...
from funding_utils_local import fund_npc_accounts_custom_coin_eth, fund_npc_accounts_native_coin_eth
...
from utils import TxType, decode_eth_private_key_to_address, eth_sign_tx_dynamic_pk,  read_eth_keys, write_tx_to_file_with_tx_type
...
def sign_ERC20_transfer_tx(tx_and_pk: tuple[dict, str]):
...
async def generate_and_write_swap_txs_parallel(f):
...
async def generate_txs():
    with open(f"./transaction_batches/eth_coin_intra_multiworker_{num_of_txs // K}k.txt", "wb") as f:
        await generate_and_write_swap_txs_parallel(f)
