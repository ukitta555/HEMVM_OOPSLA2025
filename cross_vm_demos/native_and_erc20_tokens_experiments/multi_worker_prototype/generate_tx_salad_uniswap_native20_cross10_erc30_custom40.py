...
from funding_utils_local import approve_eth_custom_coin, fund_npc_accounts_custom_coin_aptos, fund_npc_accounts_custom_coin_eth, fund_npc_accounts_native_coin_aptos, fund_npc_accounts_native_coin_eth
...
from local_abis import eth_coin_abi, vault_abi, old_router_abi
...
def generate_transfer_txs_move(
    ...
    def move_cross_uniswap_case(coin: float):
        return 0 <= coin < 0.2

    def move_intra_custom_coin_case(coin: float):
        return 0.2 <= coin < 1 
    ...
    for idx in range(move_txs_counter):
        ...
        if move_intra_custom_coin_case(salad_random_coin):
        ...
        elif move_cross_uniswap_case(salad_random_coin):
...
def generate_transfer_txs_parallel_eth(
    ...
    def eth_uniswap_case(coin: float):
        return 0 <= coin < 0.4

    def eth_erc20_case(coin: float):
        return 0.4 <= coin < 1
    ...
    for idx in range(eth_tx_count):
        ...
        unsent_txn = None
        tx_type = None 
        if eth_erc20_case(salad_random_coin):
            receiver_pk = sender_pk
            while receiver_pk == sender_pk:
        ...
        elif eth_uniswap_case(salad_random_coin):
...
async def generate_txs():
    ...
    with open(f"./transaction_batches/salad_uniswap_intra20_cross10_erc30_custom40_{num_of_txs // K}k.txt", "wb") as f:
        for _ in range(num_of_txs):
