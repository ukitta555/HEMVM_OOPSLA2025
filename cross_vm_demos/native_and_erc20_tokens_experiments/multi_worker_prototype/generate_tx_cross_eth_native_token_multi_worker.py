...
from utils import TxType, decode_eth_private_key_to_address, eth_sign_tx_dynamic_pk, eth_sign_tx_genesis, read_eth_keys, read_move_keys, write_tx_to_file_with_tx_type
...
async def generate_txs():
    with open(f"transaction_batches/eth_native_token_cross_multiworker_{num_of_txs // K}k.txt", "wb") as f:
...
async def generate_and_write_swap_txs_parallel(f):
    ...
    for idx in range(num_of_txs):
        ...
        receiver_move_cross = Account.load_key(receiver_pk)
        ...
        unsent_txn = ProxyERC20.functions \
            .sendETHCrossSpace(HexBytes(str(receiver_move_cross.address()))) \
            .build_transaction(
                {
                    "from": sender_eth,
                    'value': 10 ** 18,
