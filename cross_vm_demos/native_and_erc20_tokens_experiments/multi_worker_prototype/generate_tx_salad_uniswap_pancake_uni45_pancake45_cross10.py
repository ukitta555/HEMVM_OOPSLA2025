...
def generate_transfer_txs_parallel_eth(
    ...
    for idx in range(eth_tx_count):
        ...
        if eth_intra_case(salad_random_coin):
            ...
            unsent_txn = MoveSwapRouter.functions.swapExactTokensForTokens(
                coin_1,
                coin_2,
                1000 * 10 ** 10,
                1,
...
async def generate_txs():
    ...
    with open(f"./transaction_batches/salad_uniswap_pancake_uni45_pancake45_cross10_{num_of_txs // K}k.txt", "wb") as f:
        for _ in range(num_of_txs):
