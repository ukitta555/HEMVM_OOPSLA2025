...
from local_abis import eth_coin_abi, vault_abi
...
def generate_transfer_txs_move(
    ...
    for idx in range(move_txs_counter):
        ...
        if move_intra_case(salad_random_coin):
            ...
            receiver_address = bytes.fromhex(eth_addresses[random.choice(eth_private_keys)][2:])

            transaction_arguments = [
                TransactionArgument(receiver_address, Serializer.to_bytes),
            ...
            move_tx_types.append(TxType.CROSS_APTOS_TX)
...
def generate_transfer_txs_parallel_eth(
    ...
    VaultERC20Contract = w3.eth.contract(
        address=Web3.to_checksum_address("0x7d3d6e9f5ab582112c5cdbd712e808b2a4eafa5d"),
        abi=vault_abi
    )
    ...
    for idx in range(eth_tx_count):
        ...
        if eth_nonces.get(sender_eth) is None:
            eth_nonces[sender_eth] = w3.eth.get_transaction_count(sender_eth) # to speed up the generation and not query the tx count for each of the wallets
        
        salad_random_coin = random.uniform(0, 1)        
        
        unsent_txn = None
        tx_type = None 
        if eth_intra_case(salad_random_coin):
            receiver_pk = sender_pk
            while receiver_pk == sender_pk:
                receiver_pk = random.choice(eth_private_keys)
            receiver_eth = eth_addresses[receiver_pk]
            ...
            receiver_move_pk = random.choice(move_private_keys)
            receiver_move = move_accounts[receiver_move_pk]
            
            unsent_txn = VaultERC20Contract.functions.deposit(
                str(receiver_move.address()), # bytes32 moveReceiver
...
async def generate_txs():
    ...
    with open(f"./transaction_batches/salad_ERC_custom_coin_e55_m35_c10_{num_of_txs // K}k.txt", "wb") as f:
        for _ in range(num_of_txs):
