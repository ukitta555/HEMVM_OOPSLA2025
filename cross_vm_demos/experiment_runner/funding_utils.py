...
from abis import eth_coin_abi, portal_abi
...
async def fund_wallets(experiment):
    from runner_multi_worker_prototype import Experiment
    experiment: Experiment = experiment
    try:
        if experiment.funding_type == FundingType.NATIVE_ETH:
            await fund_npc_accounts_native_coin_eth(
                eth_keys_path="./keys/private_keys_ethereum.txt",
            )
        elif experiment.funding_type == FundingType.NATIVE_APTOS:
            await fund_npc_accounts_native_coin_aptos(
                aptos_keys_path="./keys/private_keys_aptos.txt"
            )
        elif experiment.funding_type == FundingType.NATIVE_BOTH:
            await fund_npc_accounts_native_coin_eth(
                eth_keys_path="./keys/private_keys_ethereum.txt",
            )
            await fund_npc_accounts_native_coin_aptos(
        ...
        elif experiment.funding_type == FundingType.NATIVE_AND_CUSTOM_ETH:
            await fund_npc_accounts_native_coin_eth(
                eth_keys_path="./keys/private_keys_ethereum.txt",
            )
            await fund_npc_accounts_custom_coin_eth(
                eth_keys_path="./keys/private_keys_ethereum.txt",
                only_eth_coin_deployed=experiment.only_eth_coin_deployed,
                erc_20_cross_setup=experiment.erc_20_cross_setup
            )  
        elif experiment.funding_type == FundingType.NATIVE_AND_CUSTOM_APTOS:
            await fund_npc_accounts_native_coin_aptos(
                aptos_keys_path="./keys/private_keys_aptos.txt"
            )
        ...
        elif experiment.funding_type == FundingType.NATIVE_AND_CUSTOM_BOTH:
            await fund_npc_accounts_native_coin_eth(
                eth_keys_path="./keys/private_keys_ethereum.txt"
            )
            await fund_npc_accounts_native_coin_aptos(
            ...
            print("Starting to fund accounts with custom coins.")
            await fund_npc_accounts_custom_coin_eth(
                eth_keys_path="./keys/private_keys_ethereum.txt",
                only_eth_coin_deployed=experiment.only_eth_coin_deployed,
                erc_20_cross_setup=experiment.erc_20_cross_setup
            ...
            print("Finished funding accounts with custom coins!")
        elif experiment.funding_type == FundingType.UNISWAP_EXPERIMENT:            
            await fund_npc_accounts_native_coin_eth(
                eth_keys_path="./keys/private_keys_ethereum.txt"
            )
            await fund_npc_accounts_native_coin_aptos(
            ...
            await fund_npc_accounts_custom_coin_eth(
                eth_keys_path="./keys/private_keys_ethereum.txt",
                only_eth_coin_deployed=experiment.only_eth_coin_deployed, # False
                erc_20_cross_setup=experiment.erc_20_cross_setup # True
            )
            await fund_npc_accounts_custom_coin_aptos(
                aptos_keys_path="./keys/private_keys_aptos.txt",
                erc_20_cross_setup=experiment.erc_20_cross_setup # True
            ...
            #  TODO: remove these calls & regenerate transactions 
        ...
        elif experiment.funding_type == FundingType.UNISWAP_CROSS_EXPERIMENT:
            await fund_npc_accounts_native_coin_eth(
                eth_keys_path="./keys/private_keys_ethereum.txt"
            )
            await fund_npc_accounts_native_coin_aptos(
        ...
        elif experiment.funding_type == FundingType.PANCAKE_EXPERIMENT:
            await fund_npc_accounts_native_coin_aptos(
                aptos_keys_path="./keys/private_keys_aptos.txt"
            )
            await fund_npc_accounts_custom_coin_aptos(
        ...
        elif experiment.funding_type == FundingType.PANCAKE_CROSS_EXPERIMENT:
            await fund_npc_accounts_native_coin_eth(
                eth_keys_path="./keys/private_keys_ethereum.txt"
            )
            await fund_npc_accounts_native_coin_aptos(
            ...
            await approve_eth_custom_coin_pancakeswap_cross(
                eth_keys_path="./keys/private_keys_ethereum.txt"
            )
        elif experiment.funding_type == FundingType.MIX_UNISWAP_EXPERIMENT:
            await fund_npc_accounts_native_coin_eth(
                eth_keys_path="./keys/private_keys_ethereum.txt"
            )
            await fund_npc_accounts_native_coin_aptos(
        ...
        elif experiment.funding_type == FundingType.MIX_PANCAKE_EXPERIMENT_NATIVE_ONLY:
            await fund_npc_accounts_native_coin_eth(
                eth_keys_path="./keys/private_keys_ethereum.txt"
            )
            await fund_npc_accounts_native_coin_aptos(
        ...
        elif experiment.funding_type == FundingType.MIX_PANCAKE_EXPERIMENT_NATIVE_AND_CROSS:
            await fund_npc_accounts_native_coin_eth(
                eth_keys_path="./keys/private_keys_ethereum.txt"
            )
            await fund_npc_accounts_native_coin_aptos(
        ...
        elif experiment.funding_type == FundingType.MIX_UNI_PANCAKE_NATIVE_EXPERIMENT:
            await fund_npc_accounts_native_coin_eth(
                eth_keys_path="./keys/private_keys_ethereum.txt"
            )
            await fund_npc_accounts_native_coin_aptos(
        ...
        elif experiment.funding_type == FundingType.MIX_UNI_PANCAKE_CROSS_EXPERIMENT:
            await fund_npc_accounts_native_coin_eth(
                eth_keys_path="./keys/private_keys_ethereum.txt"
            )
            await fund_npc_accounts_native_coin_aptos(
