...
import {address, Contract} from "js-conflux-sdk";
...
task("balanceMapped", "Prints an account's balance")
    .addParam("account", "The account's address")
    .setAction(async ({account}) => {
      const crossSpaceCall = conflux.InternalContract("CrossSpaceCall");

      const result = await crossSpaceCall.mappedBalance(account);
...
const config: HardhatUserConfig = {
  ...
  networks: {
    localhost: {
      ...
      accounts: ["0xfafafafafafafafafafafafafafafafafafafafafafafafafafafafafafafafa"],
      chainId: 1,
    }
  }
};
